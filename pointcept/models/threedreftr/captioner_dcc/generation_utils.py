...
def beam_search_decode(transformer: Callable, **kwargs) -> Tensor:
    ...
    return OrderedDict({
        'output_ids': torch.gather( #  torch.Size([256, 32])
            output_ids.long(), 1, 
            output_scores.argmax(-1, keepdim=True).unsqueeze(1).repeat(1, 1, max_length)
        ).squeeze(1),
        'output_scores': output_scores, #  torch.Size([256, 5])
        'beam_output_ids': output_ids.long() # torch.Size([256, 5, 32])
    })
...
def generation(transformer: Callable, **kwargs):
    ...
    kwargs['max_length'] = kwargs.get('max_length', 32) # 32 
    kwargs['early_stopping'] = kwargs.get('early_stopping', True) # True
    kwargs['num_beams'] = kwargs.get('num_beams', None) # 5
    kwargs['eos_token_id'] = kwargs.get('eos_token_id', -1) # 3
    kwargs['restore_prefix'] = kwargs.get('restore_prefix', False) # False
    kwargs['encoder_hidden_states'] = kwargs.get('encoder_hidden_states', None) #torch.Size([256, 128, 256]), nproposals, context_size, channel
    
    # import pdb; pdb.set_trace()
    input_ids = kwargs.get('input_ids', None) # None
    inputs_embeds = kwargs.get('inputs_embeds', None) #torch.Size([256, 1, 256]), nproposals, nprefix, channel
    ...
    if kwargs['num_beams'] is None: # 5
        # batch x max_length
        outputs = greedy_decode(transformer, **kwargs)
    else:
        outputs = beam_search_decode(transformer, **kwargs) #odict_keys(['output_ids', 'output_scores', 'beam_output_ids'])
                                                            #  torch.Size([256, 32]), torch.Size([256, 5]), torch.Size([256, 5, 32])
    ## post-processing, adding prefix if necessary
    if kwargs['restore_prefix'] is True: #False
