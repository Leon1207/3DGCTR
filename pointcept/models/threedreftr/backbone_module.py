...
class Pointnet2Backbone(nn.Module):
    ...
    def forward(self, pointcloud, offset, bss):
        ...
        xyz = pointcloud[..., 0:3].contiguous().view(bss, -1, 3).float()
        features = pointcloud[..., 3:].view(bss, -1, 3).transpose(-1, -2).contiguous().float()

        # --------- 4 SET ABSTRACTION LAYERS ---------
        xyz, features, fps_inds = self.sa1(xyz, features)
        end_points['sa1_inds'] = fps_inds       # ([B, 2048])
        end_points['sa1_xyz'] = xyz.float()             # ([B, 2048, 3])
        end_points['sa1_features'] = features.float()   # ([B, 128, 2048])

        xyz, features, fps_inds = self.sa2(xyz.float(), features.float())  # this fps_inds is just 0,1,...,1023
        end_points['sa2_inds'] = fps_inds       # 1024
        end_points['sa2_xyz'] = xyz.float()
        end_points['sa2_features'] = features.float()

        xyz, features, fps_inds = self.sa3(xyz.float(), features.float())  # this fps_inds is just 0,1,...,511
        end_points['sa3_xyz'] = xyz.float()
        end_points['sa3_features'] = features.float()

        xyz, features, fps_inds = self.sa4(xyz.float(), features.float())  # this fps_inds is just 0,1,...,255
        end_points['sa4_xyz'] = xyz.float()
        end_points['sa4_features'] = features.float()

        # --------- 2 FEATURE UPSAMPLING LAYERS --------
        features = self.fp1(end_points['sa3_xyz'], end_points['sa4_xyz'], end_points['sa3_features'],
                            end_points['sa4_features'])
        features = self.fp2(end_points['sa2_xyz'], end_points['sa3_xyz'], end_points['sa2_features'], features.float())
        end_points['fp2_features'] = features.float()           # ([B, 288, 1024])
        end_points['fp2_xyz'] = end_points['sa2_xyz'].float()   # ([B, 1024, 3])
